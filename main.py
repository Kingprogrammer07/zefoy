...
class Zefoy:
    ...
    {Fore.RED}Created by »»» t.me/java_strong                                   
    {Fore.BLUE}URL ni kiriting: """)
        a = str(a)
        try:
            if a.startswith("https://www.tiktok.com/"):
                try:
                    # tekshiruv
                    b = a.split("video/")[1]
                    f = b.split("?")[0]
                    if f.isdigit():
                        d = a.split("?")[0]
                        y = {"link": f"{d}"}
                        with open("config.json", "w") as dikt:
                            dump(y, dikt, indent = 4)
                        print(f"{Fore.BLUE} Muvoffaqiyatli saqlandi !!")
                        break
                    else:
                        print(f"{Fore.GREEN} URL ni boshqattan kiriting !!")
                        continue
                except:
                    print(f"{Fore.RED} Videoni Havolasini olishda Xatolik !!!\nQayta Urining!")
                    continue
            
            elif a == "1":
                print(f"{Fore.GREEN} Avval Saqlangan Link ishlamoqda !!")
                break
            
            else:
                print(f"{Fore.RED} Siz TikTok URL kiritmadingiz !!!")
                continue
        except:
            continue
